//+------------------------------------------------------------------+
//|                                              NDCB_XAU_v1.mq5     |
//|     XAUUSD H1 — NY Data-Hour Compression Breakout (NDCB) v1.0    |
//|                                                                  |
//|  Validated structural edge (8/8 years 2017-2024 PASS at CR=0.70):|
...
//+------------------------------------------------------------------+
#property copyright "NDCB-XAU EA v1.0 — pending stop @ NY data-hour break"
...
int OnInit()
{
   ...
   LogInfo(StringFormat("NDCB-XAU v1.0 Init | %s | State=%s", _Symbol, StateName(g_state)));
   ...
   if(g_starting_equity > 0)
   {
      ...
      if(abs_dd > InpTrailingDDPct / 100.0)
      {
         ...
         else
         {
            ...
            g_recovery_start = 0;
            LogCritical("Recovery complete — resuming");
         }
      }
   }
...
//+------------------------------------------------------------------+
//| Cancel pending orders if entry window expired                    |
//+------------------------------------------------------------------+
void CheckPendingExpiry()
{
   int cur_hour = GetBrokerHour(TimeCurrent());
   if(cur_hour <= InpEntryEndHour) return;
...
void OnTick()
{
   ...
   // ── State machine ─────────────────────────────────────────────────────
   if(g_state == NDCB_WAITING)
   {
      // After comp window closes (i.e., 14:00 bar has closed), evaluate
      if(hour >= InpEntryStartHour && hour < InpEntryEndHour)
      {
         EvaluateCompressionAndPlaceOrders();
      }
      else if(hour >= InpEntryEndHour)
      {
         // Past entry window — no-op for the day
         g_state = NDCB_COMP_DONE;
         SaveState();
      }
   }
   else if(g_state == NDCB_PENDING)
   {
      CheckPendingFill();   // promotes to TRADE_ACTIVE if any position exists
      if(g_state == NDCB_PENDING) CheckPendingExpiry();
   }
   else if(g_state == NDCB_TRADE_ACTIVE)
   {
      ManageActiveTrade();
   }
}
