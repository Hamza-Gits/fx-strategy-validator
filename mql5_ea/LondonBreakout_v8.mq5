//+------------------------------------------------------------------+
//|                                          LondonBreakout_v8.mq5  |
//|     GBPUSD London Breakout EA — PARITY ORACLE (v8)              |
//|                                                                  |
//|  v8 = v7 + per-bar decision trace logger                        |
//|  Purpose: find exact bar where MQL5 diverges from Python        |
//|                                                                  |
//|  New: InpTraceLogging = true writes                              |
//|    decision_trace_mql5_GBPUSD_YYYY.csv to MT5 Files folder      |
//|  Schema matches phase2_python_trace.py for direct diff.         |
//|                                                                  |
...
//+------------------------------------------------------------------+
#property copyright "London Breakout EA v8 - parity oracle"
...
input string InpCSVFileName       = "LondonBreakout_v8_trades.csv";

input group "=== Parity Oracle Trace (Phase 2) ==="
input bool   InpTraceLogging      = true;
input string InpTraceFileName     = "decision_trace_mql5_GBPUSD_2023.csv";
...
string        gv_prefix             = "";
int           g_trace_file          = INVALID_HANDLE;
...
//+------------------------------------------------------------------+
//| PARITY ORACLE TRACE LOGGER (Phase 2)                            |
//| Writes one row per London/post-Asian bar — identical schema to  |
//| phase2_python_trace.py for line-by-line diff.                   |
//+------------------------------------------------------------------+
void EnsureTraceHeader()
{
   if(!InpTraceLogging) return;
   g_trace_file = FileOpen(InpTraceFileName, FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_SHARE_READ, ',');
   if(g_trace_file == INVALID_HANDLE)
   {
      Print("Trace file open failed: ", InpTraceFileName);
      return;
   }
   FileWrite(g_trace_file,
   ...
   FileFlush(g_trace_file);
}

void WriteTraceRow(datetime bar_time_gmt, double bar_close,
                   string trend_dir, double asian_high, double asian_low,
                   double asian_range_pips, string range_ok,
                   string in_london, string signal, string skip_reason,
                   double entry_price, double sl, double tp)
{
   if(!InpTraceLogging || g_trace_file == INVALID_HANDLE) return;

   MqlDateTime dt; TimeToStruct(bar_time_gmt, dt);
   string date_str = StringFormat("%04d-%02d-%02d", dt.year, dt.mon, dt.day);
   string time_str = TimeToString(bar_time_gmt, TIME_DATE|TIME_SECONDS);

   FileWrite(g_trace_file,
      date_str,
      time_str,
      DoubleToString(bar_close, 5),
      g_w1_ema_value > 0 ? DoubleToString(g_w1_ema_value, 5) : "",
      trend_dir,
      asian_high > 0 ? DoubleToString(asian_high, 5) : "",
      asian_low  > 0 ? DoubleToString(asian_low,  5) : "",
      asian_range_pips > 0 ? DoubleToString(asian_range_pips, 1) : "",
      range_ok,
      in_london,
      signal,
      skip_reason,
      entry_price > 0 ? DoubleToString(entry_price, 5) : "",
...
int OnInit()
{
   gv_prefix = "LB8_" + _Symbol + "_";   // v8 prefix — prevents stale v7 state
   ...
   LogInfo(StringFormat("EA v8.0 Init | %s | PipSize=%.5f | PipValue=%.2f | State=%s",
...
void OnDeinit(const int reason)
{
   ...
   {
      FileFlush(g_trace_file);
      FileClose(g_trace_file);
      g_trace_file = INVALID_HANDLE;
      LogInfo(StringFormat("Trace file closed: %s", InpTraceFileName));
   }
   SaveStateToGlobals();
...
void CheckBarCloseEntry()
{
   ...
   datetime bar_gmt_time = bars[0].time + GetGMTOffsetSeconds();
   ...
   double asian_range_pips = range / g_pip_size;
   string in_london       = "YES";
   string range_ok_str    = (asian_range_pips >= InpMinRangePips && asian_range_pips <= InpMaxRangePips) ? "YES" : "NO";

   int trend = GetTrendDirection();
   string trend_str = (trend == 1) ? "LONG" : (trend == -1) ? "SHORT" : "FLAT";

   if(trend == 0)
   {
      LogInfo("Trend filter: skip today (ambiguity/no data)");
      WriteTraceRow(bar_gmt_time, prevClose, trend_str,
                    g_asian_high, g_asian_low, asian_range_pips, range_ok_str,
                    in_london, "NONE", "TREND_FILTER_NO_DATA", 0, 0, 0);
      g_trade_taken_today = true;
      SaveStateToGlobals();
      return;
   }
   ...
   if((trend == 1 || trend == 2) && prevClose > g_asian_high)
   {
      ...
      if(trade.Buy(lots, _Symbol, 0, sl, tp, "LB-Long"))
      {
         ...
         WriteTraceRow(bar_gmt_time, prevClose, trend_str,
                       g_asian_high, g_asian_low, asian_range_pips, range_ok_str,
                       in_london, "LONG", "", g_entry_price, sl, tp);
      ...
      else
      {
         ...
         WriteTraceRow(bar_gmt_time, prevClose, trend_str,
                       g_asian_high, g_asian_low, asian_range_pips, range_ok_str,
                       in_london, "NONE", "BUY_ORDER_FAILED", 0, 0, 0);
      }
   }
   ...
   if(!entered && (trend == -1 || trend == 2) && prevClose < g_asian_low)
   {
      ...
      if(trade.Sell(lots, _Symbol, 0, sl, tp, "LB-Short"))
      {
         ...
         WriteTraceRow(bar_gmt_time, prevClose, trend_str,
                       g_asian_high, g_asian_low, asian_range_pips, range_ok_str,
                       in_london, "SHORT", "", g_entry_price, sl, tp);
      ...
      else
      {
         ...
         WriteTraceRow(bar_gmt_time, prevClose, trend_str,
                       g_asian_high, g_asian_low, asian_range_pips, range_ok_str,
                       in_london, "NONE", "SELL_ORDER_FAILED", 0, 0, 0);
      }
   }

   // No breakout this bar
   if(!entered && !g_trade_taken_today)
   {
      WriteTraceRow(bar_gmt_time, prevClose, trend_str,
                    g_asian_high, g_asian_low, asian_range_pips, range_ok_str,
                    in_london, "NONE", "NO_BREAKOUT", 0, 0, 0);
   }
}
