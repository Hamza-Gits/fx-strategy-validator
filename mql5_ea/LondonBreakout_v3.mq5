...
//+------------------------------------------------------------------+
#property copyright "London Breakout EA v3 - 2-3x progressive sizing"
...
input double InpTrailingDDPct      = 8.0;   // Trailing DD halt threshold (relaxed for personal acct vs prop firm 4%)
...
datetime      g_ea_start_time    = 0;       // v3: tracks Phase 1 start for progressive risk
int           g_current_phase    = 1;       // v3: 1, 2, or 3
...
void LoadStateFromGlobals()
{
   if(GlobalVariableCheck(gv_prefix + "State"))
   {
      ...
      // v3: Restore progressive risk state
      if(GlobalVariableCheck(gv_prefix + "EAStartTime"))
      {
         ...
         LogInfo(StringFormat("Progressive risk restored: started %s, day %d, phase %d",
                 TimeToString(g_ea_start_time, TIME_DATE), (int)days, g_current_phase));
      }
...
//+------------------------------------------------------------------+
//| PROGRESSIVE RISK SIZING (v3 — Council Iter 1 winning rule)       |
//+------------------------------------------------------------------+
double GetCurrentRiskPct()
{
   if(!InpUseProgressiveRisk)
   ...
   if(g_ea_start_time == 0)
   {
      ...
      GlobalVariableSet(gv_prefix + "EAStartTime", (double)g_ea_start_time);
      LogInfo(StringFormat("Progressive risk: EA start time recorded: %s",
              TimeToString(g_ea_start_time, TIME_DATE)));
   }

   long days_elapsed = (long)((GetGMTNow() - g_ea_start_time) / 86400);

   int new_phase;
   double risk_pct;
   if(days_elapsed < InpPhase1DurationDays)
   {
      new_phase = 1;
      risk_pct = InpRiskPhase1Pct;
   }
   else if(days_elapsed < InpPhase1DurationDays + InpPhase2DurationDays)
   {
      new_phase = 2;
      risk_pct = InpRiskPhase2Pct;
   }
   else
   {
      new_phase = 3;
      risk_pct = InpRiskPhase3Pct;
   }

   if(new_phase != g_current_phase)
   {
      LogCritical(StringFormat("PHASE TRANSITION: Phase %d -> Phase %d | Risk: %.2f%% | Day %d",
                  g_current_phase, new_phase, risk_pct, (int)days_elapsed));
...
double CalculateLots(double stop_pips)
{
   ...
   double current_risk_pct = GetCurrentRiskPct();
   double risk_dollars = equity * (current_risk_pct / 100.0);
   ...
   LogInfo(StringFormat("Sizing: phase=%d risk=%.2f%% equity=$%.2f risk_$=$%.2f stop=%.1f raw=%.4f final=%.2f",
           g_current_phase, current_risk_pct, equity, risk_dollars, stop_pips, lots_raw, lots));
   return lots;
}
